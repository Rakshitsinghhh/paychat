  ...
  return (
    <WagmiProvider config={config}>
      <QueryClientProvider client={queryClient}>
        ...
        {!walletConnected ? (
          <Appbar setWalletConnected={setWalletconnected} />
          ...
          <div>
            <div className="flex min-h-screen bg-black">
              {/* Main Content Area */}
              <div
                className={`bg-gray-900 shadow-2xl transition-all duration-700 ease-in-out ${(isRegistered || isLoggedIn) ? 'w-1/3' : 'w-full'
                  }`}
              >
                <div className="p-6 max-w-md mx-auto">
                  ...
                  <div className="mb-4">
                    <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 ${isRegistered || isLoggedIn
                      ...
                      receivers.map((name: string, index: number) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left p-4 border-b border-gray-700 transition-all duration-200 hover:bg-gray-700 ${selectedContact === name ? 'bg-gray-700 border-l-4 border-l-blue-500' : ''
                              }`}
                          >
                        ...
                        {contactMessages.length === 0 ? (
                          ...
                          contactMessages.map((message: Message, index: number) => {
                            ...
                            return (
                              ...
                              >
                                <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${isFromMe
                                  ? 'bg-blue-600 text-white'
                                  : 'bg-gray-700 text-white'
                                  }`}>
                                  <div className="break-words">{message.content}</div>
                                  <div className={`text-xs mt-1 ${isFromMe ? 'text-blue-200' : 'text-gray-400'
                                    }`}>
